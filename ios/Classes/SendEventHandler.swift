...
struct SendEventHandler {
  ...
  // Helper to extract price from parameters, handling multiple types
  private static func extractPrice(from parameters: [String: Any]) -> NSNumber? {
    guard !(parameters["price"] is NSNull) else {
      return nil
    }
  ...
  // Helper to extract quantity from parameters, handling multiple types
  private static func extractQuantity(from parameters: [String: Any]) -> Int? {
    guard !(parameters["quantity"] is NSNull) else {
      return nil
    }
  ...
  static func handle(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    if !isValidEventName(eventTypeName) {
      result(
        FlutterError(
          ...
          message:
            "Event type name contains invalid characters. Use only letters, numbers, and underscore.",
          details: nil
        ))
      return
    }
    ...
    do {
      let event: TikTokBaseEvent = createEvent(
        eventTypeName: eventTypeName, eventName: eventName, eventId: eventId, parameters: parameters
      )
  ...
  private static func createEvent(
    eventTypeName: String, eventName: String, eventId: String?, parameters: [String: Any]
  ) -> TikTokBaseEvent {
